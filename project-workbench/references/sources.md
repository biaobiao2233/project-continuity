...
- OpenCode skills: https://opencode.ai/docs/skills — global discovery includes `~/.config/opencode/skills`, `~/.claude/skills`, and `~/.agents/skills`; same-name later sources can override earlier sources.
- OpenCode rules: https://opencode.ai/docs/zh-cn/rules/ — user-level global rules live in `~/.config/opencode/AGENTS.md`; compatible versions may fall back to `~/.claude/CLAUDE.md` when the OpenCode global file is absent.
- OpenCode agents/config: https://opencode.ai/docs/zh-cn/agents/ and https://dev.opencode.ai/docs/config/ — custom agents/system prompts and config precedence are separate from AGENTS/Skill discovery; verify the installed version before relying on optional config features.
